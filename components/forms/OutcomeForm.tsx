...
export default function OutcomeForm({
	...
	const { outcomeCategories } = useSelector((state: AppState) => state.category);
	...
	const { refetch } = useGetItemsQuery({ type: "002", year})
	...
	useEffect(() => {
		...
		if(error) {
			...
			console.log("create outcome error >>", error);
		}
	...
	useEffect(() => {
		...
		if(updateError) {
			...
			console.log("update outcome error >>", updateError);
		}
	...
	return (
		<Dialog open={open} onOpenChange={setOpen}>
			...
			<DialogContent className="w-[90%] rounded-lg sm:max-w-[425px]">
				<DialogHeader>
					<DialogTitle className="text-center">{isUpdate ? "Edit" : "New"} Outcome</DialogTitle>
				</DialogHeader>
